...
public class workPanel extends JPanel{
	...
	JMenuItem thisroot=new JMenuItem("Change to root node",new ImageIcon(MainFrame.class.getResource("root_syb.png")));
	JMenuItem del_node=new JMenuItem("Delete this root",new ImageIcon(MainFrame.class.getResource("delete_syb.png")));
	
	JPopupMenu Change_edge=new JPopupMenu("Change_edge");
	JMenuItem changeNum=new JMenuItem("Change weight number",new ImageIcon(MainFrame.class.getResource("changeEdge.png")));
	JMenuItem del_edge=new JMenuItem("Delete this edge",new ImageIcon(MainFrame.class.getResource("delete_syb.png")));
	...
	public GuideFrame BFSGuide;
	public GuideFrame DFSGuide;
	...
	public workPanel(final MainFrame mf) {
		...
		BFSGuide =new GuideFrame(20,330);
		BFSGuide.addItem(Color.decode("#508006"), "Root node passed");
		BFSGuide.addItem(Color.blue, "Node passed");
		BFSGuide.addItem(Color.decode("#808080"), "Node not passed");
		BFSGuide.addItem(Color.decode("#fa8b39"), "Node going to pass");
		BFSGuide.addItem(Color.decode("#ff0000"), "Node is passing");
		BFSGuide.addItem(Color.decode("#a536f2"), "Node in checking");
		
		DFSGuide = new GuideFrame(20,330);
		DFSGuide.addItem(Color.decode("#508006"), "Root node passed");
		DFSGuide.addItem(Color.blue, "Node passed");
		DFSGuide.addItem(Color.decode("#808080"), "Node not passed");
		DFSGuide.addItem(Color.red, "Node is passing");
		...
		ActionListener action = new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				repaint();
				//repaint is to redraw, pay attention when we change something we should use repaint: v
			}
		};
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(BFS){
					BFSGuide.setPosition(startDrag.x, startDrag.y-15);
					repaint();
				}
				if(DFS){
					DFSGuide.setPosition(startDrag.x, startDrag.y-15);
					repaint();
				}
				if(edit){
					if(!showMenu){
		...
		this.addMouseMotionListener(new MouseMotionAdapter(){
			public void mouseDragged(MouseEvent e){
				...
				if(BFS){
					BFSGuide.setPosition(endDrag.x, endDrag.y-15);
					repaint();
				}
				if(DFS){
					DFSGuide.setPosition(endDrag.x, endDrag.y-15);
					repaint();
				}
			}
